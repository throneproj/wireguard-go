...
func newObfChain(spec string) (*obfChain, error) {
	...
	// A spec holding no tags at all (empty string, or text without any <...>)
	// carries no payload. Report it as "unconfigured" rather than an empty
	// chain, so the packet is skipped instead of emitting a zero-length one.
	if len(obfs) == 0 {
		return nil, nil
	}
